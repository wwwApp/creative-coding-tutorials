...
const BOUNCE = 0.92; // lower the value, less the bounce
...
export class BounceString {
	...
	animate(ctx, moveX, moveY) {
		...
		if (
			...
			// tx, ty represents the transform point/destination point
			// (i think)
			this.detect = 300;
			...
			this.detect = 10;
			let tx = this.points[1].ox; // set the dest point to original point
			let ty = this.points[1].oy; // since you're no longer in touch with the ball
			this.points[1].vx += tx - this.points[1].x;
		...
		// changes to this center point of the string
		// shows the direction of the plucing by the ball
